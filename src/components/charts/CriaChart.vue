...
const series = computed(() => {
  ...
  const eventoData = props.eventosReproductivos.map((evento) => ({
    ...
    y: 1.5, // Posición Y fija para que aparezcan abajo
    tipo_evento: evento.tipo_evento,
    color: getEventoColor(evento.tipo_evento), // Asignar color
  }))
...
const chartOptions = computed(() => ({
  ...
  stroke: {
    width: [4, 0], // Línea gruesa para CC, 0 para scatter
    curve: 'smooth',
  },
  colors: ['#008FFB'], // Color para la línea de CC (los puntos se colorean solos)
  title: {
  ...
  markers: {
    size: 0, // Ocultar marcadores de la línea
    strokeWidth: 0,
    hover: {
      size: 9,
    },
  ...
  tooltip: {
    ...
    shared: false,
    intersect: true,
    // Tooltip personalizado para mostrar la info del evento
